...
bool Tutorial03::CreateFramebuffers() {
  ...
  for (size_t i = 0; i < swap_chain_images.size(); ++i) {
    ...
    if (vkCreateFramebuffer(GetDevice(), &framebuffer_create_info, nullptr,
                            &Vulkan.Framebuffers[i]) != VK_SUCCESS) {
...
bool Tutorial03::CreateCommandBuffers() {
  ...
  if (!AllocateCommandBuffers(Vulkan.GraphicsCommandPool, image_count,
                              Vulkan.GraphicsCommandBuffers.data())) {
...
bool Tutorial03::AllocateCommandBuffers(VkCommandPool pool, uint32_t count,
...
bool Tutorial03::RecordCommandBuffers() {
  ...
  for (size_t i = 0; i < Vulkan.GraphicsCommandBuffers.size(); ++i) {
    ...
    vkCmdBeginRenderPass(Vulkan.GraphicsCommandBuffers[i],
...
void Tutorial03::ChildClear() {
  if (GetDevice() != VK_NULL_HANDLE) {
    ...
    if ((Vulkan.GraphicsCommandBuffers.size() > 0) &&
        (Vulkan.GraphicsCommandBuffers[0] != VK_NULL_HANDLE)) {
      vkFreeCommandBuffers(
          GetDevice(), Vulkan.GraphicsCommandPool,
...
Tutorial03::~Tutorial03() {
  ...
  if (GetDevice() != VK_NULL_HANDLE) {
    ...
    if (Vulkan.RenderingFinishedSemaphore != VK_NULL_HANDLE) {
      vkDestroySemaphore(GetDevice(), Vulkan.RenderingFinishedSemaphore,
                         nullptr);
    }
  }
}
