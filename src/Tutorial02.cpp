...
Tutorial02::Tutorial02() : VulkanLibrary(), Window(), Vulkan() {}
...
bool Tutorial02::CreateCommandBuffers() {
  ...
  if (vkAllocateCommandBuffers(Vulkan.Device, &cmd_buffer_allocate_info,
                               Vulkan.PresentQueueCmdBuffers.data()) !=
      VK_SUCCESS) {
...
bool Tutorial02::RecordCommandBuffers() {
  ...
  for (uint32_t i = 0; i < image_count; ++i) {
    ...
    vkBeginCommandBuffer(Vulkan.PresentQueueCmdBuffers[i],
...
void Tutorial02::Clear() {
  if (Vulkan.Device != VK_NULL_HANDLE) {
    ...
    if ((Vulkan.PresentQueueCmdBuffers.size() > 0) &&
        (Vulkan.PresentQueueCmdBuffers[0] != VK_NULL_HANDLE)) {
      vkFreeCommandBuffers(
          Vulkan.Device, Vulkan.PresentQueueCmdPool,
          static_cast<uint32_t>(Vulkan.PresentQueueCmdBuffers.size()),
          Vulkan.PresentQueueCmdBuffers.data());
...
Tutorial02::~Tutorial02() {
  ...
  if (Vulkan.Device != VK_NULL_HANDLE) {
    ...
    if (Vulkan.ImageAvailableSemaphore != VK_NULL_HANDLE) {
      vkDestroySemaphore(Vulkan.Device, Vulkan.ImageAvailableSemaphore,
                         nullptr);
    }
    if (Vulkan.RenderingFinishedSemaphore != VK_NULL_HANDLE) {
      vkDestroySemaphore(Vulkan.Device, Vulkan.RenderingFinishedSemaphore,
                         nullptr);
    }
